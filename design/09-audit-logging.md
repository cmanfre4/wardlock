...
Audit log storage evolves with adoption phases:

- **Single-operator (Adoption Phase 1-2)**: File-based. Append-only JSON lines log on the broker's filesystem (or in the StateStore alongside credential metadata).
...
At scale, audit logging is one of the control plane's responsibilities — it stays in the stateful core rather than being delegated to workers.
