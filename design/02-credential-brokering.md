...
3. **Issue** — the backend [credential provider](03-credential-providers.md) creates a scoped, time-limited credential and returns a credential bundle describing what needs to be injected.
4. **Inject** — the broker hands the bundle to the [isolation provider](01-isolation.md), which materializes the files in the target environment (localhost filesystem in early phases, container filesystem later). Secrets are placed where CLI tools expect them — they are not returned through the MCP response.
